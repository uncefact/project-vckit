...
export interface ICredentialIssuerLD extends IPluginMethodMap {
   ...
   *
   * @returns - a promise that resolves to the {@link @vckit/core-types#VerifiablePresentation} that was requested or rejects with an error
   ...
   * @param context - This reserved param is automatically added and handled by the framework, *do not override*
   *
   * @returns - a promise that resolves to the {@link @vckit/core-types#VerifiableCredential} that was requested or rejects with an error
   * if there was a problem with the input or while getting the key to sign
   *
...
export interface ICreateVerifiablePresentationLDArgs {
  ...
  /**
   * Optional. The key handle ({@link @vckit/core-types#IKey.kid | IKey.kid}) from the internal database.
   */
  keyRef?: string
